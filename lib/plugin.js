...
const PORT = 3000;
const LOG_APP_NAME = 'containership-logs';
                ...
                api.getHosts((err, hosts) => {
                    ...
                    if(targetHost) {
                        const targetContainer = _.find(targetHost.containers, (c) => {
                            return (c.name || c.application) === LOG_APP_NAME;
                        });

                        const container_ip = targetContainer.internal_container_ip || _.get(targetHost, ['address', 'private']);
                        const container_port = targetContainer.container_port || targetContainer.host_port || PORT;
                        ...
                        function cleanup(err) {
                            if (err) {
                                console.error('ERROR in the containership.logs plugin: ', err);
                            }
